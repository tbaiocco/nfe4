...
public class Movimento_Ordem_ServicoBD extends Utilitaria {
	...
	public Movimento_Ordem_ServicoED inclui(Movimento_Ordem_ServicoED ed) throws Excecoes {
		...
		if (ed.getOid_Pessoa()> 0){
			...
			sql+=",oid_Motorista" +
				 ...
				 "," + ed.getNr_Odometro() +
				 "," + ed.getNr_Odometro() + // ed.getNr_Kilometragem() + -> substituir após o calculo da km acumulada... 
    ...
    throws Excecoes{
        ...
        sql += getSQLFrom(ed);
        sql += getSQLFiltro(ed);
        //TIAGO - Manter este order by por veiculo-dt movimento por causa do relatório de abastecimentos.
    ...
    public ArrayList lista_comp_veiculo(Movimento_Ordem_ServicoED ed, String tipo) throws Excecoes 
    {
		...
		try {
		    ...
		    //Calcula as Médias por Tipo, Modelo e Tipo/Modelo.
		    sql = "SELECT " ;
		    if ("4".equals(ed.getDm_Opcao()) || "6".equals(ed.getDm_Opcao())) {
		    	sql += "tv.nm_tipo_Veiculo , " ;}
		    if ("5".equals(ed.getDm_Opcao()) || "6".equals(ed.getDm_Opcao())) {
    ...
    throws Excecoes {
        ...
        try {
            ...
            throw new Excecoes("Método clone não implementado na classe [Movimento_Ordem_ServicoED]!");
        }
        ...
        } else throw new Mensagens("Data inicial não informada!");
